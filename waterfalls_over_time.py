...
import riversound; import scipy; import os
...
#%%
#%% Diversion Dam
## mixed data quality in February 2017, though discharge does change quickly there (012)
## Pretty good data in June 2017, but bad demonstration (012)

st = obspy.Stream()
for fn in sorted(glob.glob('DiversionDam/2019*/2019*')): # 021, 034
    print(fn)
    st += obspy.read(fn)
#st = st.merge(method = 1, fill_value = 'interpolate', interpolation_samples = 100)
#st = st.split()
#%%
for date in range(15, 25):
    print(date)
    t1 = obspy.UTCDateTime(2017,6,date,9,0,0)
    t2 = obspy.UTCDateTime(2017,6,date,10,0,0)
    tr = st.select(station='021').slice(t1, t2).merge()[0]
    tr.filter('highpass', freq = 1, corners = 4)
    print(tr)
    s = riversound.spectrum(tr)
    sg.append(s['median'])
...
rms = []
medfreqs = []
t1 = obspy.UTCDateTime(2019,4,24,9,0,0)
t2 = obspy.UTCDateTime(2019,7,24,10,0,0)
bitweight = 0.256/2**15 / (3.35/7 * 46e-6 * (1+49.7/0.47))
t = t1
while t < t2:
    print(t)
    st_tmp = st.select(station='034').slice(t, t+3600).merge(method = 1, fill_value = 'interpolate', interpolation_samples = 100)
    print(st_tmp)
    if (len(st_tmp) == 0) or (len(st_tmp[0]) < 90000): # skip if inadequate data
        print(f'skip {t}')
        medfreqs.append(np.nan)
...
# log y-axis
l1, = ax[0].semilogy(df.t[df.discharge > 10], df.discharge[df.discharge > 10], label = 'Discharge')
#ax[0].yaxis.set_major_locator(FixedLocator([100,200,300]))
#ax[0].yaxis.set_major_formatter(FuncFormatter(lambda y, _: f"{y:g}"))
right_y_axis = ax[0].twinx()
power = rms**2 * 2*np.pi*distance**2/impedance
...
#right_y_axis.yaxis.set_major_locator(FixedLocator([2, 5, 10, 20]))
#right_y_axis.yaxis.set_major_formatter(FuncFormatter(lambda y, _: f"{y:g}"))
ax[0].set_xlim([pd.Timestamp('20190424', tz = 'UTC'), pd.Timestamp('20190725', tz = 'UTC')]) 
ax[0].set_ylim([200/F, 200])
ax[0].set_ylabel(r'Discharge (m$^3$/s)')
ax[0].set_title('A. Palouse Falls Discharge and Acoustic Power', loc = 'left')
ax[0].legend([l1, l2], ['Discharge', 'Acoustic Power'])


riversound.image(np.log10(np.array(sg)[w,:]), t[w], s['freqs'], log_y = True, qmin = 0.3, ax = ax[1])
ax[1].plot(t[w], np.log10(medfreqs[w]), color = 'black')
ax[1].set_ylim([np.log10(2), np.log10(25)])
ax[1].set_yticks(np.log10([2,5,10,20]), [2,5,10,20])

ax[1].set_ylabel('Frequency (Hz)')
ax[1].set_xlabel('Date')
ax[1].set_xlim([pd.Timestamp('20190424', tz = 'UTC'), pd.Timestamp('20190725', tz = 'UTC')]) 
ax[1].set_title('B. Palouse Falls Spectrogram', loc = 'left')
...
plt.close('all')
plt.subplot(3,1,1)
plt.semilogy(df.t, df.Over_Div_Dam)
plt.xlim([pd.Timestamp('20190424'), pd.Timestamp('20190725')]) # MDT
plt.ylim([1e3, 1e4])
plt.subplot(3,1,3)
riversound.image(np.log10(np.array(sg)), np.arange(15, 25), s['freqs'], log_y = True, qmin = 0.3)
plt.ylim([np.log10(2.5), np.log10(30)])
