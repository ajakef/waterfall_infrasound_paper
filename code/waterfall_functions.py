...
def spectrum(tr, criterion_function = 'default', runmed_radius_t = 0,
             ...
             kurtosis_threshold = 0.5, window = 'fft_taper'):
    ...
    window : str
        ...
        Default 'fft_taper' is obspy's name for the 10% cosine taper used in
        (McNamara and Buland, 2004), and common for measuring background noise.
    ...
    if criterion_function == 'default':
        def criterion_function(x): return (kurtosis(x) < kurtosis_threshold) & (np.std(x) > 1e-17)
    
    if window == 'fft_taper':
        window = obspy.signal.spectral_estimation.fft_taper(np.ones(nfft))
